use std::{fs, io::{Read, Write, Seek, SeekFrom, Result, Cursor}, mem::size_of, path};
use bytemuck::Pod;
...
impl<A: Sized> VarSize for BitFlags<A> 
where A: enumflags2::_internal::RawBitFlags {
    fn size(&self) -> usize {
        size_of::<A>()
    }
}
...
impl<A> Readable for BitFlags<A>
where
    A: BitFlag,
    <A as enumflags2::_internal::RawBitFlags>::Numeric: Readable,
{
    fn read_bin<R: Read>(reader: R) -> Result<Self> {
        let val = <A as enumflags2::_internal::RawBitFlags>::Numeric::read_bin(reader)?;
        Ok(BitFlags::from_bits_truncate(val))
    }
}
